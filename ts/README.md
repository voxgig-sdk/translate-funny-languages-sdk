...
`client.Translator()` — each with a small set of operations (`load`, `create`)
...
| `tester(testopts?, sdkopts?)` | `TranslateFunnyLanguagesSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `TranslateFunnyLanguagesSDK.test(testopts?, sdkopts?)` | `TranslateFunnyLanguagesSDK` | Create a test-mode client. |
...
| `client` | `client(): TranslateFunnyLanguagesSDK` | Return the parent SDK client. |
...
├── src/
│   ├── TranslateFunnyLanguagesSDK.ts        # Main SDK class
