...
//     (\`new TranslateFunnyLanguagesSDK(...)\` / \`TranslateFunnyLanguagesSDK.test(...)\` become
//     \`TranslateFunnyLanguagesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TranslateFunnyLanguagesSDK'
...
const TEST_SEED = {"entity":{"translator":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TranslateFunnyLanguagesSDK(...)\` and any
// \`TranslateFunnyLanguagesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TranslateFunnyLanguagesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TranslateFunnyLanguagesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
