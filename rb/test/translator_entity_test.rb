...
class TranslatorEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TRANSLATE_FUNNY_LANGUAGES_TEST_TRANSLATOR_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    translator_ref01_data = Helpers.to_map(translator_ref01_data_result.respond_to?(:data_get) ? translator_ref01_data_result.data_get : translator_ref01_data_result)
...
def translator_basic_setup(extra)
  ...
  entid_env_raw = ENV["TRANSLATE_FUNNY_LANGUAGES_TEST_TRANSLATOR_ENTID"]
