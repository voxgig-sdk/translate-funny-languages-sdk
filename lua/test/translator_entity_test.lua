...
describe("TranslatorEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TRANSLATE_FUNNY_LANGUAGES_TEST_TRANSLATOR_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    translator_ref01_data = helpers.to_map(type(translator_ref01_data_result) == 'table' and translator_ref01_data_result.data_get and translator_ref01_data_result:data_get() or translator_ref01_data_result)
...
function translator_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("TRANSLATE_FUNNY_LANGUAGES_TEST_TRANSLATOR_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["TRANSLATE_FUNNY_LANGUAGES_TEST_TRANSLATOR_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["TRANSLATE_FUNNY_LANGUAGES_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["TRANSLATE_FUNNY_LANGUAGES_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
