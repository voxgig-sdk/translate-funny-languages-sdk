...
require("translate-funny-languages_types")
...
local TranslateFunnyLanguagesSDK = {}
TranslateFunnyLanguagesSDK.__index = TranslateFunnyLanguagesSDK
...
TranslateFunnyLanguagesSDK._make_feature = _make_feature


function TranslateFunnyLanguagesSDK.new(options)
  local self = setmetatable({}, TranslateFunnyLanguagesSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TranslateFunnyLanguagesSDK:options_map()
...
function TranslateFunnyLanguagesSDK:get_utility()
  return Utility.copy(self._utility)
end


function TranslateFunnyLanguagesSDK:get_root_ctx()
  return self._rootctx
end


function TranslateFunnyLanguagesSDK:prepare(fetchargs)
...
function TranslateFunnyLanguagesSDK:direct(fetchargs)
...
function TranslateFunnyLanguagesSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function TranslateFunnyLanguagesSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "TranslateFunnyLanguagesSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function TranslateFunnyLanguagesSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TranslateFunnyLanguagesSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "TranslateFunnyLanguagesSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function TranslateFunnyLanguagesSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TranslateFunnyLanguagesSDK.new(sdkopts)
  sdk.mode = "test"
...
return TranslateFunnyLanguagesSDK
