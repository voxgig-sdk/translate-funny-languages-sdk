...
import (
	...
	omni "github.com/voxgig-sdk/translate-funny-languages-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TranslateFunnyLanguagesError); is {
				out := map[string]any{
					"name":    "TranslateFunnyLanguagesError",
...
func omniCtx(arg any, client *sdk.TranslateFunnyLanguagesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TranslateFunnyLanguagesSDK); is {
			client = live
