...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TRANSLATEFUNNYLANGUAGES_TEST_LIVE") == "TRUE" ||
		os.Getenv("TRANSLATEFUNNYLANGUAGES_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TRANSLATEFUNNYLANGUAGES_TEST_EXPLAIN"); explain != "" {
