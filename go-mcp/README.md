# translate-funny-languages-mcp

[MCP](https://modelcontextprotocol.io) server exposing the TranslateFunnyLanguages SDK as
two agent tools — `translate-funny-languages_list` and `translate-funny-languages_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/translate-funny-languages-mcp)
make build

# 2. Provide credentials via the environment
export TRANSLATE_FUNNY_LANGUAGES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user translate-funny-languages \
  -- /absolute/path/to/translate-funny-languages-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./translate-funny-languages-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// translate-funny-languages_list: first page of records
{ "entity": "translator" }
{ "entity": "translator", "query": { } }

// translate-funny-languages_load: one record by id
{ "entity": "translator", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/translate-funny-languages-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user translate-funny-languages \
     -- "$PWD"/dist/*/translate-funny-languages-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `translate-funny-languages_list` and `translate-funny-languages_load` tools now appear
   in new sessions. Ask the agent to *"list translator using translate-funny-languages"*
   and it calls `translate-funny-languages_list` with `{"entity":"translator"}`.
...
```sh
export TRANSLATE_FUNNY_LANGUAGES_APIKEY=sk_live_xxx            # API key
export TRANSLATE_FUNNY_LANGUAGES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `translate-funny-languages_list` tool
...
### Call the `translate-funny-languages_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `translate-funny-languages_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `translate-funny-languages_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TRANSLATE_FUNNY_LANGUAGES_APIKEY` | API key sent with every request. |
| `TRANSLATE_FUNNY_LANGUAGES_BASE` | Optional override of the API base URL. |
...
```sh
./translate-funny-languages-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"translate-funny-languages_load","arguments":{"entity":"translator","query":{"id":1}}}}'
```
