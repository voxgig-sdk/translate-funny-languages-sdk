# TranslateFunnyLanguages SDK test runner
...
class TranslateFunnyLanguagesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TranslateFunnyLanguagesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TranslateFunnyLanguagesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TranslateFunnyLanguagesTestRunner.getenv("TRANSLATE_FUNNY_LANGUAGES_TEST_LIVE")
        override = TranslateFunnyLanguagesTestRunner.getenv("TRANSLATE_FUNNY_LANGUAGES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TranslateFunnyLanguagesTestRunner.getenv(key)
        ...
        explain = TranslateFunnyLanguagesTestRunner.getenv("TRANSLATE_FUNNY_LANGUAGES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TranslateFunnyLanguagesTestRunner._test_control is not None:
            return TranslateFunnyLanguagesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TranslateFunnyLanguagesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TranslateFunnyLanguagesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TranslateFunnyLanguagesTestRunner.load_test_control()
...
def load_env_local():
    TranslateFunnyLanguagesTestRunner.load_env_local()


def env_override(m):
    return TranslateFunnyLanguagesTestRunner.env_override(m)


def entity_data(v):
    return TranslateFunnyLanguagesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TranslateFunnyLanguagesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TranslateFunnyLanguagesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TranslateFunnyLanguagesTestRunner.load_test_control()


def live_client_options():
    return TranslateFunnyLanguagesTestRunner.live_client_options()


def live_delay_ms():
    return TranslateFunnyLanguagesTestRunner.live_delay_ms()
