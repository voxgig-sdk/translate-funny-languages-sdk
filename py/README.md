# TranslateFunnyLanguages Python SDK



The Python SDK for the TranslateFunnyLanguages API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Translator()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/translate-funny-languages-sdk/releases)) or
...
```python
client = TranslateFunnyLanguagesSDK.test(testopts, sdkopts)
```
...
| `Translator` | `(data) -> TranslatorEntity` | Create a Translator entity instance. |
...
```
py/
├── translatefunnylanguages_sdk.py         -- Main SDK module
...
The main module (`translatefunnylanguages_sdk`) exports the SDK class.
