...
//    only - a typed TranslateFunnyLanguagesContext there would make every ctx
...
final class TranslateFunnyLanguagesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TranslateFunnyLanguagesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TranslateFunnyLanguagesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TranslateFunnyLanguagesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TranslateFunnyLanguagesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TranslateFunnyLanguagesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?TranslateFunnyLanguagesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TranslateFunnyLanguagesError(is_string($code) ? $code : '', $msg);
    }
