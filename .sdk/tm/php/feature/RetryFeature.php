...
// TranslateFunnyLanguages SDK retry feature
...
class TranslateFunnyLanguagesRetryFeature extends TranslateFunnyLanguagesBaseFeature
{
    ...
    public function init(TranslateFunnyLanguagesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TranslateFunnyLanguagesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TranslateFunnyLanguagesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TranslateFunnyLanguagesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
