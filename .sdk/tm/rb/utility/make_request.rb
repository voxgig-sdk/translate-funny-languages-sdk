...
module TranslateFunnyLanguagesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TranslateFunnyLanguagesResponse.new({})
    ...
    if fetch_err
      ...
      response = TranslateFunnyLanguagesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TranslateFunnyLanguagesResponse.new(fetched)
    else
