...
class TranslatorEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TRANSLATE_FUNNY_LANGUAGES_TEST_TRANSLATOR_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $translator_ref01_data = Helpers::to_map(is_object($translator_ref01_data_result) && method_exists($translator_ref01_data_result, 'data_get') ? $translator_ref01_data_result->data_get() : $translator_ref01_data_result);
...
function translator_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["translator01", "translator02", "translator03"] as $k) {
    ...
    $entid_env_raw = getenv("TRANSLATE_FUNNY_LANGUAGES_TEST_TRANSLATOR_ENTID");
