# translate-funny-languages-cli

boru-driven command-line client **and** interactive REPL for the TranslateFunnyLanguages
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/translate-funny-languages-cli)
make build

# 2. See usage (words, entities, env vars)
./translate-funny-languages-cli --help

# 3. Provide credentials once, via the environment
export TRANSLATE_FUNNY_LANGUAGES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./translate-funny-languages-cli load 1 translator            # {id:1} shorthand
./translate-funny-languages-cli load '{id:1}' translator       # explicit match map

# 5. Override the API base URL for a single call
TRANSLATE_FUNNY_LANGUAGES_BASE=https://api.example.com ./translate-funny-languages-cli load 1 translator

# 6. No arguments -> interactive REPL
./translate-funny-languages-cli
translate-funny-languages> load 1 translator
translate-funny-languages> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/translate-funny-languages-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/translate-funny-languages-cli load 1 translator
   ```
...
```sh
./translate-funny-languages-cli load 1 translator          # scalar shorthand for {id:1}
./translate-funny-languages-cli load '{id:1}' translator     # explicit match map
```
...
./translate-funny-languages-cli load 1 translator
```
...
Run with no arguments to open a REPL (prompt `translate-funny-languages>`). Each line is
evaluated as its own boru expression:

```text
$ ./translate-funny-languages-cli
translate-funny-languages> load 1 translator
translate-funny-languages> /help
translate-funny-languages> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `translator`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/translate-funny-languages-cli`. |
...
(`./translate-funny-languages-cli <expr>`) and interactively (the REPL), and expressions compose the
